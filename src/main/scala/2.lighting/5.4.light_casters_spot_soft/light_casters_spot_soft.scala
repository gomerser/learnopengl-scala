...
@main def main(): Unit =
  ...
  val lightingShader = Shader(
    "2.lighting/5.4.light_casters.vs",
    "2.lighting/5.4.light_casters.fs"
  )
  val lightCubeShader =
    Shader("2.lighting/5.4.light_cube.vs", "2.lighting/5.4.light_cube.fs")
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    lightingShader.setFloat(
      "light.outerCutOff",
      math.cos(Math.toRadians(17.5f)).toFloat
    )
