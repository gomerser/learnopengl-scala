...
@main def main(): Unit =
  ...
  // -------------------------
  val shader = Shader(
    "5.advanced_lighting/3.1.3.shadow_mapping.vs",
    "5.advanced_lighting/3.1.3.shadow_mapping.fs"
  )
  val simpleDepthShader = Shader(
    "5.advanced_lighting/3.1.3.shadow_mapping_depth.vs",
    "5.advanced_lighting/3.1.3.shadow_mapping_depth.fs"
  )
  val debugDepthQuad = Shader(
    "5.advanced_lighting/3.1.3.debug_quad.vs",
    "5.advanced_lighting/3.1.3.debug_quad_depth.fs"
  )
  ...
  val borderColor = Array(1.0f, 1.0f, 1.0f, 1.0f)
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // lightPos.x = math.sin(glfwGetTime).toFloat * 3.0f
    // lightPos.z = math.cos(glfwGetTime).toFloat * 2.0f
    // lightPos.y = 5.0f + math.cos(glfwGetTime).toFloat * 1.0f
    ...
    val near_plane = 1f
    val far_plane = 7.5f
    // val lightProjection = Matrix4f().perspective(math.toRadians(45.0f).toFloat, SHADOW_WIDTH.toFloat / SHADOW_HEIGHT.toFloat, near_plane, far_plane) // note that if you use a perspective projection matrix you'll have to change the light position as the current light position isn't enough to reflect the whole scene
    val lightProjection =
