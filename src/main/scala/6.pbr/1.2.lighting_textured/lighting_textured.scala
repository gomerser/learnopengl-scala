...
@main def main(): Unit =
  ...
  // --------------------------
  val albedo = loadTexture("/textures/pbr/rusted_iron/albedo.png")
  val normal = loadTexture("/textures/pbr/rusted_iron/normal.png")
  val metallic = loadTexture("/textures/pbr/rusted_iron/metallic.png")
  val roughness = loadTexture("/textures/pbr/rusted_iron/roughness.png")
  val ao = loadTexture("/textures/pbr/rusted_iron/ao.png")

  // lights
  // ------
  val lightPositions = ArrayBuffer[Vector3f]()
  lightPositions += Vector3f(0.0f, 0.0f, 10.0f)
  val lightColors = ArrayBuffer[Vector3f]()
  lightColors += Vector3f(150.0f, 150.0f, 150.0f)
  val nrRows = 7
  val nrColumns = 7
  val spacing = 2.5f
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (row <- 0 until nrRows) {
      for (col <- 0 until nrColumns) {
        ...
        shader.setMat3("normalMatrix", Matrix3f(model).invert.transpose)
        renderSphere()
      }
    }
    ...
    for (i <- 0 until lightPositions.length) {
      val newPos = lightPositions(i).add(
        Vector3f(math.sin(glfwGetTime() * 5.0).toFloat * 5.0f, 0.0, 0.0)
      )
      // val newPos = lightPositions(i)
      shader.setVec3(s"lightPositions[$i]", newPos)
      shader.setVec3(s"lightColors[$i]", lightColors(i))
