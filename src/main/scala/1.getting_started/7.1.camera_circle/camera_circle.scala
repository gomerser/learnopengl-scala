...
@main def main(): Unit =
  ...
  // ------------------------------------
  val ourShader =
    Shader("1.getting_started/7.1.camera.vs", "1.getting_started/7.1.camera.fs")
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    val view = Matrix4f()
    val radius = 10.0f
    val camX = (Math.sin(glfwGetTime()) * radius).toFloat
    val camZ = (Math.cos(glfwGetTime()) * radius).toFloat
    view.lookAt(
      Vector3f(camX, 0.0f, camZ), // camera position
      Vector3f(0.0f, 0.0f, 0.0f), // look-at target
