...
@main def main(): Unit =
    ...
    val fragmentShaderOrange = glCreateShader(GL_FRAGMENT_SHADER) // the first fragment shader that outputs the color orange
    val fragmentShaderYellow = glCreateShader(GL_FRAGMENT_SHADER) // the second fragment shader that outputs the color yellow
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, false, 3 * java.lang.Float.BYTES, 0L)	// Vertex attributes stay the same
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, false, 0, 0L) // because the vertex data is tightly packed we can also specify 0 as the vertex attribute's stride to let OpenGL figure it out
    glEnableVertexAttribArray(0)
