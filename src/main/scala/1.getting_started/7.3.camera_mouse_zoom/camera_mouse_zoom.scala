...
var cameraUp = Vector3f(0.0f, 1.0f, 0.0f)
...
var lastX: Float = SCR_WIDTH / 2.0f
var lastY: Float = SCR_HEIGHT / 2.0f
var fov: Float = 45.0f
...
def processInput(window: Long): Unit = {
  ...
  if glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS then
    cameraPos.fma(cameraSpeed, cameraFront) // cameraPos += cameraFront * speed
  if glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS then
    cameraPos.fma(-cameraSpeed, cameraFront) // cameraPos -= cameraFront * speed
  if glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS then
    val left = Vector3f(cameraFront).cross(cameraUp).normalize()
    cameraPos.fma(-cameraSpeed, left) // cameraPos -= left * speed
  if glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS then
    val right = Vector3f(cameraFront).cross(cameraUp).normalize()
    cameraPos.fma(cameraSpeed, right) // cameraPos += right * speed
}
...
def mouse_callback(window: Long, xposIn: Double, yposIn: Double): Unit = {
  ...
  val front = new Vector3f(
    Math
      .cos(Math.toRadians(yaw.toDouble))
      .toFloat * Math.cos(Math.toRadians(pitch.toDouble)).toFloat,
    Math.sin(Math.toRadians(pitch.toDouble)).toFloat,
    Math.sin(Math.toRadians(yaw.toDouble)).toFloat * Math
      .cos(Math.toRadians(pitch.toDouble))
      .toFloat
  )
