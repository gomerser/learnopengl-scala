...
@main def main(): Unit =
  ...
  // ------------------------------------
  val ourShader =
    Shader("1.getting_started/6.1.coordinate_systems.vs", "1.getting_started/6.1.coordinate_systems.fs")
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    var projection = Matrix4f().identity()
    model = model.rotate(Math.toRadians(-55.0f).toFloat, 1.0f, 0.0f, 0.0f)
