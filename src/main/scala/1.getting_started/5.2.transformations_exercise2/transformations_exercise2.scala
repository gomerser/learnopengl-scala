...
@main def main(): Unit =
  ...
  val ourShader = Shader(
    "1.getting_started/5.2.transform.vs",
    "1.getting_started/5.2.transform.fs"
  )
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // ---------------------
    val scaleAmount = math.sin(glfwGetTime()).toFloat
    transform = Matrix4f()
      .identity() // reset it to identity matrix
      .translate(0.5f, -0.5f, 0.0f)
