...
@main def main(): Unit =
  ...
  val ourShader =
    Shader(
      "1.getting_started/6.2.coordinate_systems.vs",
      "1.getting_started/6.2.coordinate_systems.fs"
    )
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    var model = Matrix4f()
      .identity()
      .rotate(glfwGetTime().toFloat, 0.5f, 1.0f, 0.0f)
    var view = Matrix4f()
      .identity()
