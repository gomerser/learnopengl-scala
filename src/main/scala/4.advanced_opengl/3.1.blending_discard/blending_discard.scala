...
@main def main(): Unit =
  ...
  val shader =
    Shader(
      "4.advanced_opengl/3.1.blending.vs",
      "4.advanced_opengl/3.1.blending.fs"
    )
  ...
  val transparentTexture = loadTexture("/textures/grass.png")

  // transparent vegetation locations
  // --------------------------------
  val vegetation = Array[Vector3f](
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for i <- 0 until vegetation.size do {
      model = new Matrix4f().translate(vegetation(i))
      shader.setMat4("model", model)
      glDrawArrays(GL_TRIANGLES, 0, 6)
    }
