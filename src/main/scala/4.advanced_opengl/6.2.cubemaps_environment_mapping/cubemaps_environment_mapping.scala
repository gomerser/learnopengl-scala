...
@main def main(): Unit =
  ...
  val shader =
    Shader(
      "4.advanced_opengl/6.2.cubemaps.vs",
      "4.advanced_opengl/6.2.cubemaps.fs"
    )
  val skyboxShader =
    Shader(
      "4.advanced_opengl/6.2.skybox.vs",
      "4.advanced_opengl/6.2.skybox.fs"
    )
  ...
  val skyboxVertices = Array[Float](
    // positions
  ...
  // skybox VAO
  val skyboxVAO = glGenVertexArrays()
  val skyboxVBO = glGenBuffers()
  glBindVertexArray(skyboxVAO)
  glBindBuffer(GL_ARRAY_BUFFER, skyboxVBO)
  val skyboxVertexBuf = MemoryUtil.memAllocFloat(skyboxVertices.length)
  skyboxVertexBuf.put(skyboxVertices).flip()
  glBufferData(GL_ARRAY_BUFFER, skyboxVertexBuf, GL_STATIC_DRAW)
  MemoryUtil.memFree(skyboxVertexBuf)
  glEnableVertexAttribArray(0)
  glVertexAttribPointer(
    0,
    3,
  ...
  val cubemapTexture = loadCubemap(faces)
...
// -Z (back)
// -------------------------------------------------------
def loadCubemap(faces: Seq[String]): Int = {
  val textureID = glGenTextures()
  ...
  usingStack { stack =>
    ...
    for i <- 0 until faces.length do {
      val imgBytes = loadResourceAsTexture(faces(i))
      val data = stbi_load_from_memory(imgBytes, w, h, nrComponents, 3)
      if data != null then
        val width = w.get(0)
        val height = h.get(0)
        glTexImage2D(
