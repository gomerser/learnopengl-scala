...
// camera
val camera = Camera(Vector3f(0.0f, 0.0f, 55.0f))
...
@main def main(): Unit =
  ...
  val shader =
    Shader(
      "4.advanced_opengl/10.2.instancing.vs",
      "4.advanced_opengl/10.2.instancing.fs"
    )
  ...
  val amount = 1000
  val modelMatrices = Array.ofDim[Matrix4f](amount)
  ...
  for (i <- 0 until amount) {
    ...
    val scale = (rng.nextInt(20)) / 100.0 + 0.05
    model.scale(scale.toFloat)

    // 3. rotation: add random rotation around a (semi)randomly picked rotation axis vector
    val rotAngle = rng.nextInt(360)
    model.rotate(rotAngle.toFloat, 0.4f, 0.6f, 0.8f)
